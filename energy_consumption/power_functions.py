...
def quaternion_to_euler(df):
    ...
    '''
    t0 = +2.0 * (df['orientation_w'] * df['orientation_x'] + df['orientation_y'] *  df['orientation_z'])
    t1 = +1.0 - 2.0 * (df['orientation_x'] * df['orientation_x'] + df['orientation_y'] * df['orientation_y'])
    df['phi'] = np.arctan2(t0, t1)

    t2 = +2.0 * (df['orientation_w'] * df['orientation_y'] - df['orientation_z'] * df['orientation_x'])
    t2[t2 > +1] = 1
    t2[t2 < -1] = -1
    df['theta'] = np.arcsin(t2)

    t3 = +2.0 * (df['orientation_w'] *  df['orientation_z'] + df['orientation_x'] * df['orientation_y'])
    t4 = +1.0 - 2.0 * (df['orientation_y'] * df['orientation_y'] +  df['orientation_z'] *  df['orientation_z'])
...
def VairBody(df):
    ...
    Vair = np.asarray([df['velocity_x'],df['velocity_y'],df['velocity_z']]) #ground velocity
    ...
    vx = df['velocity_x']*ct*cps + df['velocity_y']*ct*sps - df['velocity_z']*st
    vy = df['velocity_x']*(sph*st*cps-cph*sps) + df['velocity_y']*(sph*st*sps+cph*cps) + df['velocity_z']*(sph*ct)
    vz = df['velocity_x']*(cph*st*cps+sph*sps) + df['velocity_y']*(cph*st*sps-sph*sps) + df['velocity_z']*(cph*ct)
    ...
    vai = np.sqrt(df['velocity_x']**2 + df['velocity_y']**2 + df['velocity_z']**2)
